...
def test_multitrack_reference_is_confirmed_rendered_and_repeatable() -> None:
    ...
    assert first["step_count"] == 26
    assert first["trace_count"] == 26
    assert first["visual_trace_count"] >= 4
    assert first["transition_trace_count"] == 2
    assert first["automation_trace_count"] >= 3
    assert first["mask_trace_count"] >= 1
    ...
    assert first["current_transition_count"] == 2
